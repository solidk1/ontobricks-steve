...
class AppRoleService:
    ...
    def resolve_role(
        ...
        try:
            ...
            # The Postgres store swallows its own errors and returns [], so this
            # only fires for a store that does raise. Both land in the same place.
        ...
        if best is ROLE_NONE and not admin_exists:
            # No admin is recorded anywhere. Either the registry has not been
            # initialised (``app_roles`` does not exist, and the store reports that
            # as an empty list rather than an error) or the table is genuinely
            # empty. Both deadlock the same way: initialising the registry lives
            # behind an admin check, so with no admin nobody can create the table
            # that would grant one.
            #
            # ``ONTOBRICKS_BOOTSTRAP_ADMIN`` exists to break exactly that. It is
            # honoured only while no admin exists, so a deliberate revoke is never
            # undone, and it grants to one operator-configured address, never via
            # a group.
            boot = cls.bootstrap_admin()
            if boot and boot.strip().lower() == (email or "").strip().lower():
                logger.warning(
                    "resolve_role: no admin recorded in app_roles; granting admin "
                    "to the configured ONTOBRICKS_BOOTSTRAP_ADMIN %s so the "
                    "registry can be initialised",
                    email,
                )
                return ROLE_ADMIN
